...
auto setup_window() -> void
{
	...
	// Try to find Discord overlay first, fallback to creating custom window
	MyWnd = FindWindowA("IceCustomShadowWindow", "IceCustomShadowWindow");
	
	// If Discord overlay not found, try other common overlay windows
	if (!MyWnd) {
		MyWnd = FindWindowA("ShadowWindow", NULL);
	}
	if (!MyWnd) {
		MyWnd = FindWindowA("MedalOverlayClass", "MedalOverlay");
	}
	
	// If still not found, create our own overlay window
	if (!MyWnd) {
		WNDCLASSA wc = { 0 };
		wc.lpfnWndProc = WndProc;
		wc.hInstance = GetModuleHandleA(0);
		wc.lpszClassName = "AzureOverlay";
		wc.style = CS_VREDRAW | CS_HREDRAW;
		RegisterClassA(&wc);

		MyWnd = CreateWindowExA(WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_LAYERED,
			"AzureOverlay", "Azure Overlay",
			WS_POPUP,
